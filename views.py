...
def loginpage(request):
  ...
  return render(request,'bonge/loginpage.html')


def index(request):
    
    return render(request,'bonge/index.html')
def team(request):
   if request.method=='POST':
      uname1=request.POST.get('name')
      password=request.POST.get('password')
      user=authenticate(request,username=uname1,password=password)
      if user is not None:
         return redirect('index')
      else:
...
def symptom(request):
   ...
   if request.method =="POST":
      return redirect('bonge/home.html')
   
   return render(request,'bonge/symptom.html')

    
def doctor(request):
    
        if 'department1' in request.POST:
            department = request.POST['department1']
            doc=Doctor.objects.filter(department=department)
        else:
            doc = []    

        return render(request,'bonge/doctor.html',{'doc':doc})
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        # Training dataset
        data = pd.read_csv(os.path.join('C:/Users/HP/Downloads/aqua/bonge/bonge/dataset', 'Training.csv'))
        print(len(data))
        ...
        # Testing dataset
        dataset = pd.read_csv(os.path.join('C:/Users/HP/Downloads/aqua/bonge/bonge/dataset', 'Training.csv'))
...
def predictdisease(request):
    ...
    if (h == 'yes'):
        ...
        return render(request, 'bonge/result1.html', context)
...
def predict(request):
    return render(request, 'bonge/result1.html')
